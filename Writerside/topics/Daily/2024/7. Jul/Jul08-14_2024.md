...
4. **Collision Handling:**
   ...
   Here we handle three cases:
    - The left robot has lower health: Decrease its health and remove the right robot (health set to 0).
      Since the right robot is on the stack, it's also removed from the simulation.
    - The right robot has lower health: Decrease its health and remove the left robot (health set to 0).
    - Both robots have the same health: Remove both robots (healths set to 0).
      Since the right robot is on the stack, it's also removed.
