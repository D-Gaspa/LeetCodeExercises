...
The central concept of this solution is to leverage a linear scan approach with in-place modifications to efficiently
track k-bit flips across the array.
This method allows for optimized handling of overlapping flips while minimizing space usage.

- **Linear Scan:** The algorithm processes each element once, making decisions based on the current state and previous
  flips.
- **In-place Modifications:** The original array is cleverly used to implicitly store flip information, eliminating the
  need for additional data structures.
- **Expected State Tracking:** An `expected_state` variable is maintained to determine whether a flip is needed at each
  position.
- **Virtual Flips:** Instead of actually performing k-bit flips, the algorithm simulates their effect by toggling
  individual values.

> **Key Insight:**
> By using the input array itself to store flip information and maintaining an expected state, the
> solution achieves $O(1)$ space complexity while still handling overlapping flips efficiently.
>
{style="note"}
...
1. **Initialization and Edge Case Handling:**
   ...
   ```
   The function starts with an optimization for `k=1`, simply counting the zeros.
   It then initializes the flip count and sets the expected state to 1 (as we want all elements to be 1 eventually).

2. **Main Loop - Linear Scan:**
   ```python
   for index in range(k, len(nums)):
   ```
   This loop processes each element of the array starting from the k-th position, deciding whether to flip based on
   previous operations.

3. **Checking and Updating Flip Status:**
   ```python
   if nums[index - k] != expected_state:
       expected_state = 1 - expected_state
       flip_count += 1
   ```
   If the element `k` positions ago doesn't match the expected state, it means a flip was performed there.
   The expected state is toggled, and the flip count is incremented.

4. **Simulating Flip Effect:**
   ```python
   if expected_state == 0:
       nums[index] = 1 - nums[index]
   ```
   If the expected state is 0, the current element is flipped (simulating the effect of a k-bit flip without actually
   performing it).

5. **Final Check and Result Calculation:**
   ```python
   last_value = nums[-1]
   if not all(num == last_value for num in nums[-k:]):
       return -1
   ...
   return flip_count + (1 if last_value != expected_state else 0)
   ```
   The function checks if the last `k` elements are all the same.
   If not, it's impossible to make all elements 1. Finally, it returns the total flip count,
   potentially adding one more flip if the last element doesn't match the expected state.
...
- $O(n)$, where $n$ is the length of the input array `nums`.
  ...
  with an additional $O(k)$ operation at the end to check the last $k$ elements.
  Since $k \leq n$, the overall time complexity remains $O(n)$.

**Space Complexity:**

- $O(1)$, as the algorithm uses only a constant amount of extra space regardless of the input size.
  It cleverly uses the input array itself to store flip information,
