...
![june24-2024-ex1.png](june24-2024-ex1.png)
...
def bstToGst2(root: TreeNode) -> TreeNode:
    ...
    The time complexity is O(n), where `n` is the number of nodes in the tree,
    as each node is visited exactly once. The space complexity is O(h), where
    `h` is the height of the tree, due to the stack used for traversal. In the
    worst case of an unbalanced tree, this could be O(n) (skewed tree), but for
    a balanced BST, it would be O(log n).
    """
...
def bstToGst3(root: TreeNode) -> TreeNode:
    ...
    (rightmost node of the left subtree) to the current node, allowing
