...
## June 7 -> 350. Intersection of Two Arrays II {collapsible="true" default-state="collapsed"}
...
### Approach 1: Two-Pointer on Sorted Arrays {id="approach_d5_1" collapsible="true" default-state="expanded"}
...
1. 4 appears in both arrays (once in `nums1` and twice in `nums2`), so it's included once in the intersection
2. 9 appears in both arrays (once in `nums1` and twice in `nums2`), so it's included once in the intersection
3. 5 and 8 appear in only one array each, so they're not included in the intersection

---

### Approach 2: Hash Table (Counter) Approach {id="approach_d5_2" collapsible="true" default-state="expanded"}
