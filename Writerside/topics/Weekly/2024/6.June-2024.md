...
def numberOfSpecialSubstrings1(s: str) -> int:
    ...
    The space complexity is O(k), where `k` is the size of the alphabet (26
    for lowercase English letters), as the set stores at most `k` unique
    characters. Since O(26) is a constant, the space complexity can be
    considered O(1).
    """
    window_chars = set()
    left_index = 0
