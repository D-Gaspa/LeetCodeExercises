...
def splitBST3(root: Optional[TreeNode], 
    ...
    which could potentially skip subtrees, this method always processes all
    nodes. The space complexity is O(1) as it uses only a constant amount of
...
- $O(n)$, where $n$ is the number of nodes in the tree.
  This is because the algorithm visits every node in the tree exactly once, regardless of the tree's structure.
  Unlike the previous two approaches, which could potentially skip subtrees, this method always processes all nodes.
...
This approach trades off the potential efficiency gains of skipping subtrees (as seen in the previous two methods) for
guaranteed linear time complexity and constant space complexity.
It's particularly helpful for large trees or in memory-constrained environments,
